...
// Reuse the same datasets and structure as Pre-Assessment Part 1
...
const setQuestionCounts = [1, 1, 2, 1, 2, 2, 1, 1, 2, 2];
...
const PostAssessment: React.FC<SectionPageProps> = ({ user, onBack }) => {
            ...
            <div className="figure-wrapper">
              ...
              <svg className="chart-svg" viewBox="0 0 420 280" preserveAspectRatio="xMidYMid meet" aria-label="Temperature vs Electricity Consumption">
                {(() => {
                  ...
                  for (let i=0;i<4;i++) { const y = margin.top + (i+1)*(innerH/5); elements.push(<line key={`g${i}`} x1={margin.left} y1={y} x2={margin.left+innerW} y2={y} stroke="#eef3fb" strokeWidth={1} />); }
                  tempsX.forEach((x, idx) => { const y = elecY[idx]; elements.push(<circle key={`p${idx}`} cx={xScale(x)} cy={yScale(y)} r={4.5} fill="#2C4795" opacity={0.9} />); });
                  const y1 = 63.80*minX - 1390.75; const y2 = 63.80*maxX - 1390.75;
            ...
            <div className="figure-wrapper">
              ...
              <svg className="chart-svg" viewBox="0 0 420 280" preserveAspectRatio="xMidYMid meet" aria-label="Years vs Sea Level Change">
                {(() => {
                  ...
                  for (let i=0;i<4;i++) { const y = margin.top + (i+1)*(innerH/5); elements.push(<line key={`g6${i}`} x1={margin.left} y1={y} x2={margin.left+innerW} y2={y} stroke="#eef3fb" strokeWidth={1} />); }
                  yearsSince2015.forEach((x, idx) => { const y = seaLevelChange[idx]; elements.push(<circle key={`s6${idx}`} cx={xScale(x)} cy={yScale(y)} r={4.5} fill="#2C4795" opacity={0.9} />); });
                  const y1 = 2.89*minX - 0.09; const y2 = 2.89*maxX - 0.09;
            ...
            <div className="figure-wrapper">
              ...
              <svg className="chart-svg" viewBox="0 0 420 280" preserveAspectRatio="xMidYMid meet" aria-label="Water Quality vs Tourist Arrivals">
                {(() => {
                  ...
                  for (let i=0;i<4;i++) { const y = margin.top + (i+1)*(innerH/5); elements.push(<line key={`g7${i}`} x1={margin.left} y1={y} x2={margin.left+innerW} y2={y} stroke="#eef3fb" strokeWidth={1} />); }
                  waterQualityX.forEach((x, idx) => { const y = touristArrivalsY[idx]; elements.push(<circle key={`s7${idx}`} cx={xScale(x)} cy={yScale(y)} r={4.5} fill="#2C4795" opacity={0.9} />); });
            ...
            <div className="figure-wrapper">
              ...
              <svg className="chart-svg" viewBox="0 0 420 280" preserveAspectRatio="xMidYMid meet" aria-label="Temperature vs Water Consumption">
                {(() => {
                  ...
                  for (let i=0;i<4;i++) { const y = margin.top + (i+1)*(innerH/5); elements.push(<line key={`g10${i}`} x1={margin.left} y1={y} x2={margin.left+innerW} y2={y} stroke="#eef3fb" strokeWidth={1} />); }
                  waterConsumptionY.forEach((x, idx) => { const y = dailyTempX[idx]; elements.push(<circle key={`s10${idx}`} cx={xScale(x)} cy={yScale(y)} r={4.5} fill="#2C4795" opacity={0.9} />); });
  ...
  const renderPart2 = () => (
    <section className="pre-assessment-part2">
      <h2>Part 2: End-of-Lesson Survey (SACCIQ)</h2>
    ...
    <div className="portal-container">
      <header className="portal-header">
        <div className="header-left">
          <span className="header-badge badge--post">✅</span>
          <div className="header-texts">
            <h1 className="portal-title">Post Assessment</h1>
      ...
      <main className="portal-content">
        {isLockedAfterSubmit ? (
          ...
          part2Submitted ? (
            <section className="pre-assessment-part2">
              <h2>Post-Assessment Completed</h2>
